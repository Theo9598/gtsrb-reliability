...
\section{Data}
We use the Hugging Face GTSRB dataset \texttt{tanganke/gtsrb}. Dataset URL: \url{https://huggingface.co/datasets/tanganke/gtsrb}.

It contains 26,640 training images and 12,630 official test images over 43 classes. All model comparisons use the same data protocol: the original training split is divided once into a 22,644-image training subset and a 3,996-image validation subset using a stratified split with seed 242 and validation fraction 0.15. The official 12,630-image clean test split is never used for training or validation checkpoint selection; it is used only for final evaluation. Images are resized to 96 by 96 pixels and normalized using ImageNet mean and standard deviation to match the pretrained ResNet18 backbone.
...
\section{Results}
All three main models were trained for 25 epochs. The custom CNN trained from scratch reached 94.09\% validation accuracy and 74.07\% clean test accuracy. The same CNN trained with RandAugment and MixUp reached 87.86\% validation accuracy and 75.30\% clean test accuracy. Robust training therefore improves clean accuracy by 1.23 percentage points, although macro F1 is slightly lower, which suggests that the gain is not evenly distributed across all 43 classes. The final ResNet18 model trained for 25 epochs on an NVIDIA GeForce RTX 2080 Ti and reached 99.97\% validation accuracy, 98.76\% clean test accuracy, and 98.33\% macro F1. These results are comparable because every row uses the same stratified training/validation split and the same official clean test set; only the model and training method change.
...
\midrule
Custom CNN & No & No & Val 94.09; Test 74.07; F1 61.21 & Course baseline \\
Custom CNN + RandAugment/MixUp & No & Yes & Val 87.86; Test 75.30; F1 59.13 & Slight clean-accuracy gain \\
...
\caption{Custom CNN, robustly trained CNN, and robustly trained ResNet18 comparison using 25 epochs for each model.}
\end{figure}

Table 2 reports the main comparison directly. The first row is the self-built course model. The second row isolates the effect of adding RandAugment and MixUp to the same CNN architecture for the same number of epochs. Robust training improves clean-test accuracy slightly, from 74.07\% to 75.30\%, but macro F1 decreases from 61.21\% to 59.13\%. This means the robustly trained CNN is not uniformly better across classes. The third row shows that the same robust training idea is much more effective when paired with a pretrained ResNet18 backbone, improving clean-test accuracy by 24.69 percentage points over the plain custom CNN.

\begin{table}[H]
\centering
\caption{Custom CNN component ablation results from 12-epoch runs.}
\footnotesize
\begin{tabular}{p{1.45in}p{0.75in}p{2.45in}p{1.15in}}
\toprule
Model & Pretrained? & Metrics (\%) & Delta vs Full CNN \\
\midrule
Full custom CNN, 12 epochs & No & Val 78.03; Test 58.99; F1 39.26 & Baseline \\
...
The ablation in Table 3 supports the course concepts inside the custom CNN. These ablation runs use the earlier 12-epoch setting, so they are interpreted as component diagnostics rather than the main 25-epoch model comparison. Removing BatchNorm causes the largest drop, which means normalization is important for stable training in this architecture. Removing Dropout also hurts performance, showing that regularization helps the classifier generalize beyond the training split.
