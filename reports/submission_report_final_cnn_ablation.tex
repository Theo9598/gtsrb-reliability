...
GitHub appendix: [Insert GitHub repository URL]/blob/main/APPENDIX.md

Presentation recording: [Insert recording link before Gradescope submission]

\section*{Abstract}
We study traffic sign recognition as a 43-class image classification task using the German Traffic Sign Recognition Benchmark (GTSRB). To match the course emphasis on building a model, we first train our own convolutional neural network from scratch, then compare it with a stronger ImageNet-pretrained ResNet18 trained with RandAugment and MixUp. The custom CNN reaches 58.99\% clean test accuracy, while the final ResNet18 reaches 98.76\% accuracy and 98.33\% macro F1. We also run a CNN ablation study: removing BatchNorm drops clean test accuracy to 19.21\%, and removing Dropout drops it to 39.05\%, showing why these course concepts matter. Because traffic sign recognition is safety-relevant, we evaluate visual corruptions, apply temperature scaling to reduce expected calibration error from 6.36\% to 3.04\%, and deploy a Streamlit demo with upload, corruption, top-5 prediction, and Grad-CAM features.
...
\section{Data}
We use the Hugging Face GTSRB dataset \texttt{tanganke/gtsrb}, with 26,640 training images and 12,630 test images over 43 classes. We create a stratified train/validation split from the original training set and reserve the official clean test set for final evaluation. Images are resized to 96 by 96 pixels and normalized using ImageNet mean and standard deviation to match the pretrained ResNet18 backbone.
...
\section{Methodology}
The project uses two model levels so that the report includes both a course-aligned model and an advanced comparison model. The course-aligned model is a custom CNN trained from scratch, meaning all convolutional, BatchNorm, and classifier weights are randomly initialized and learned only from the GTSRB training set. The advanced model is an ImageNet-pretrained ResNet18, used to test how much transfer learning and robust augmentation improve performance beyond the self-built CNN baseline.
...
\section{Results}
The custom CNN and ablation models were trained for 12 epochs from scratch. The full custom CNN reached 78.03\% validation accuracy and 58.99\% clean test accuracy. Removing BatchNorm caused the model to learn poorly, reaching only 20.85\% validation accuracy and 19.21\% clean test accuracy. Removing Dropout was less damaging but still reduced clean test accuracy to 39.05\%. The final ResNet18 model trained for 25 epochs on an NVIDIA GeForce RTX 2080 Ti and reached 99.97\% validation accuracy, 98.76\% clean test accuracy, and 98.33\% macro F1.

\begin{table}[H]
\centering
\caption{Model comparison and ablation results.}
\footnotesize
\begin{tabular}{p{1.35in}p{0.75in}p{2.45in}p{1.25in}}
\toprule
...
ResNet18 robust & Yes & Val 99.97; Test 98.76; F1 98.33 & +39.77 pp \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=5.7in]{model_comparison_ablation.png}
\caption{Model comparison and CNN ablation study.}
\end{figure}

Table 2 reports the numerical comparison directly. The full custom CNN is the from-scratch course model, so the ablation rows measure how much performance changes when one component is removed. Removing BatchNorm drops clean-test accuracy by 39.78 percentage points, and removing Dropout drops it by 19.94 percentage points. The ResNet18 model is included as the advanced pretrained comparison and improves clean-test accuracy by 39.77 percentage points over the full custom CNN.
...
\section{Conclusion}
This project applies deep learning methodology to a real image classification problem and extends it with robustness, calibration, interpretability, and deployment analysis. The custom CNN architecture demonstrates course concepts directly: convolution, ReLU, pooling, BatchNorm, Dropout, softmax-style multiclass classification, and cross-entropy training. The ablation study shows that BatchNorm and Dropout materially affect performance. The fine-tuned ResNet18 trained with RandAugment and MixUp achieves 98.76\% clean test accuracy and 98.33\% macro F1 on GTSRB. Calibration reduces ECE from 6.36\% to 3.04\%, and corruption testing reveals the model's strengths and limitations. The main conclusion is that strong clean performance is achievable, but reliability requires explicit model comparison, ablation, robustness testing, and safety analysis.
